...

                        {
                            if (IsLoginUnique(login))
                            {
                                if (IsNicknameUnique(nickname))
                                {
                                    ...
                                    unitOfWork.Save();
                                    return "Користувача " + nickname + " успішно зареєстровано.";
                                }
                                else
                                {
                                    return "Користувач з таким нікнеймом вже зареєстрований, оберіть інший нікнейм.";
                                }
                            }
                            else
                            {
                                return "Користувач з таким логіном вже зареєстрований, оберіть інший логін.";
                            }
                        }
                        else
                        {
                            return "Пароль не може бути коротше трьох символів.";
                        }
                    }
                    else
                    {
                        return "Нікнейм не може бути коротше трьох символів.";
                    }
                }
                else
                {
                    return "Логін не може бути коротше трьох символів.";
                }
            }
        }

        public static string Login(string login, string password, out User currentUser)
        {
            using (EFUnitOfWork unitOfWork = new EFUnitOfWork())
            {
                        ...
                        if (fList.Count() == 1 || fList.Count() == 2)
                        {
                            ...
                            foreach (Message message in messageList)
                            {
                                result.Add((message.SenderID == currentUser.ID ? currentUser.Nickname : friend.Nickname) + " <> " +
                                    message.SentTimeStamp.ToString("f") + " <> " +
                                    message.MessageBody);
                            }
                        }
                        else
